...
function getFunctionBody(func) {
  let result = '';
  if (typeof func === 'function') result = func.toString();
  return result;
}
...
function getPowerFunction(exponent) {
  return function MathPow(number) {
...
function getPolynom(...args) {
  ...
  return function MathPow(x) {
    const argsLength = args.length;
    return args.reduce((result, value, index) => {
      const power = argsLength - index - 1;
      return result + value * x ** power;
    }, 0);
  };
}
...
function logger(func, logFunc) {
  return function loggingArgs(...rest) {
    const argumentsToString = rest
      .map((argument) => JSON.stringify(argument))
      .join(',');

    logFunc(`${func.name}(${argumentsToString}) starts`);
    const result = func(...rest);
    logFunc(`${func.name}(${argumentsToString}) ends`);
